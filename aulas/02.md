...
Uma das soluções mais eficazes para a documentação de APIs é a utilização da especificação [OpenAPI](https://swagger.io/specification/){:target="_blank"}. O OpenAPI fornece um padrão para descrever APIs RESTful, permitindo que desenvolvedores criem documentações precisas e testáveis de forma automática. Ferramentas como [Swagger UI](https://swagger.io/tools/swagger-ui/){:target="_blank"} e [Redoc](https://redocly.github.io/redoc/){:target="_blank"} são amplamente utilizadas para gerar visualizações interativas da documentação, baseadas na especificação OpenAPI. Estas ferramentas proporcionam uma interface fácil de navegar, onde os usuários podem não apenas ler a documentação, mas também experimentar a API diretamente na interface.

O FastAPI fornece suporte automático tanto ao swagger UI, quanto ao Redoc. Para vermos a documentação atual da nossa aplicação devemos iniciar o servidor:
...
Se acessarmos o endereço [http://127.0.0.1/docs](http://127.0.0.1/docs){:target="_blank"} iremos visualizar interface do swagger UI:

![Captura de tela da página inicial do swagger UI](assets/02_captura_de_tela_swagger_ui.png){: .center .shadow }

Nessa imagem podemos ver todos os endpoints que nossa aplicação tem, no caso `/` e ela aceita o verbo HTTP `GET`. Se clicarmos no método:

![Captura de tela da documentação do / no swagger UI](assets/02_documentação_do_swagger_ui_do_endpoint_GET.png){: .center .shadow }

Temos diversas informações importantes sendo apresentadas na documentação. Como ó código de resposta `200` que será retornado pelo servidor em caso de sucesso, o tipo de dado que será retornado pelo endpoint `application/json` e um exemplo do valor de retorno, que nesse caso é apresentado como `#!python "string"`.

Embora a documentação exista, ela está errada, nossa aplicação não retorna uma string, retorna um [JSON](#json){:target="_blank"}, mas falaremos disso um pouco mais adiante.

Outra coisa interessante do Swagger UI, é a possibilidade de executar uma requisição ao endpoint usando a própria interface web da documentação. Se clicarmos no botão `Try it out`, um novo botão `Execute` aparecerá na tela:
...
Quando clicamos no botão `Execute` o swagger se tornará um cliente da nossa aplicação e enviará uma requisição ao servidor. Com isso exibirá na tela a resposta:
