...
Quando falamos sobre transferência de dados entre cliente e servidor, sem a camada de visualização de dados, costumamos chamar erroneamente de APIs[^1].

[^1]: APIs web no vocabulário moderno são sempre marcadas como transferência de dados em formato JSON. Mais informações podem ser encontradas [no artigo do criador do REST](https://roy.gbiv.com/untangled/2008/rest-apis-must-be-hypertext-driven){:target="_blank"}.
