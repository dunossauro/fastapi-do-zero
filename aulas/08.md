...
# [REV] Tornando o projeto assíncrono
...
Um dos problemas mais comuns ao lidarmos com uma API web é a necessidade de estarmos sempre disponíveis para responder a requisições. Quando recebemos uma requisição, normalmente precisamos processá-la: validar o schema, conectar ao banco de dados, realizar algumas operações e, finalmente, retornar o recurso solicitado.

Enquanto processamos uma requisição, não conseguimos processar outra até que a primeira seja concluída. Esse fluxo de execução é chamado de [**bloqueante**](#bloqueio-de-io){:target="_blank"}, pois a aplicação fica "parada", aguardando a resposta de sistemas externos, como o banco de dados. Esse tempo de espera, conhecido como bloqueio por I/O (Input/Output), prejudica o desempenho, pois a aplicação não pode atender a novas requisições enquanto espera.
...
Uma corrotina assíncrona é um tipo especial de "função" que pode ser suspensa em determinados pontos (normalmente quando há uma operação que pode demorar, como I/O) e retomar a execução mais tarde. Isso permite que a aplicação não fique bloqueada enquanto espera por uma resposta, permitindo que o Python execute outras tarefas enquanto aguarda a conclusão da operação.
...
O `#!python await` é usado para chamar operações que podem levar algum tempo, pelo bloqueio de I/O. Isso permite que o Python "libere" o controle de volta para o [loop de eventos](#o-que-e-o-loop-de-eventos){:target="_blank"}, que pode executar outras tarefas enquanto aguarda a operação "espera" a resposta de O/O ser concluída:
...
Para que a fixture seja declarada como assíncrona, precisamos importá-la do `pytest_asyncio` e fazer a declaração com `@pytest_asyncio.fixture`. Também devemos alteramos a engine para o formato async.
...
Aqui, `engine.begin()` inicia uma transação assíncrona com o banco de dados. Dentro dessa transação, usamos o run_sync para executar a criação das tabelas, uma operação que é síncrona por natureza. Esse processo assegura que a criação das tabelas ocorra de forma não bloqueante, permitindo que o loop de eventos continue a processar outras tarefas assíncronas enquanto essa operação é realizada.

Em resumo, o uso de `run_sync` dentro de uma operação assíncrona é uma estratégia importante para integrar operações tradicionais síncronas com a arquitetura assíncrona do Python, garantindo que a performance e escalabilidade do sistema sejam preservadas.

### Ajustando o teste do banco de dados

Agora que temos as fixtures preparadas para serem executadas no contexto assíncrono, podemos iniciar nossa jornada em direção ao testes performando operações assíncronas.
...
Testes também pode ser corrotinas assíncronas, porém o pytest não os executa em um loop de eventos. Para isso seja feito, temos que indicar que um determinado teste é assíncrono. Para isso usamos a marcação `@pytest.mark.asyncio`.

Com isso feito, podemos adicionar `#!python await` em todas as chamadas diretas ao banco de dados e que apresentam bloqueios, como:

- `session.commit()`: Que persiste os dados que estão na sessão, por isso faz uma chamada ao banco de dados.
- `session.scalar()`: Que pede uma busca de dados ao banco e aguarda o seu resultado.


Isso deve ser o suficiente para que o teste seja executado com sucesso:
...
Agora que temos certeza de que o banco e as fixtures estão funcionando. Podemos passar a refatoração do projeto de fato.
...
Um exemplo disso é o [endpoint GET](#endpoint-de-get){:target="_blank"}, que está devidamente testado, mas ao olhar a cobertura:
...
Precisamos ativar a concorrência no coverage em dois casos, para threads (conversamos sobre isso [na aula 05](/05/#threads-e-conexoes){:target="_blank"}) e para o [greenlet](#configuracao-para-o-sqlalchemy-assincrono){:target="_blank"} que é adicionado pela conexão assíncrona do SQLAlchemy.
...
As migrações com alembic em teoria devem ser sempre síncronas. Afinal, você não gostaria que ele fizesse escalonamento na hora de aplicar as migrações e aplicar um pedaço de cada em pedaços diferentes de migrações de arquivos diferentes.
...
Isso acontece pois agora estamos usando uma forma assíncrona do sqlite, quando adicionamos [adicionamos](#sqlite-assincrono){:target="_blank"} "+aiosqlite" na variável de ambiente `DATABASE_URL`.
...
Com isso, as migrações deve ser executadas em erros:
