...
O `#!python await` é usado para chamar operações que podem levar algum tempo, pelo bloqueio de I/O. Isso permite que o Python "libere" o controle de volta para o [loop de eventos](#loop-de-eventos){:target="_blank"}, que pode executar outras tarefas enquanto aguarda a operação e "espera" a resposta de I/O ser concluída:
...
Embora o SQLAlchemy ofereça suporte à programação assíncrona, esse suporte não vem completo por padrão. Um dos componentes necessários, o pacote [`greenlet`](https://github.com/python-greenlet/greenlet){:target="_blank"}, não é instalado automaticamente, para manter o pacote base mais enxuto. O `greenlet` é responsável por viabilizar corrotinas cooperativas dentro do SQLAlchemy, permitindo a execução eficiente de código assíncrono.
...
Vale lembrar que nunca interagiremos diretamente com o `greenlet`. Ele é utilizado internamente pelo SQLAlchemy de forma transparente, mas é necessário para que tudo funcione corretamente.
...
Uma das funcionalidades legais do `pytest` é poder executar somente um único teste, ou um grupo deles, usando o nome do teste como base. Para isso, podemos chamar `poetry test` passando a flag `-k` seguida do nome do teste. Algo como:
...
1. Aqui podemos utilizar o nome `Session` no lugar de `SessionDep`, pois o nome `Session` não está mais no namespace.
...
```title="resposta de poetry test -k test_read"
...
```python title="fast_zero/security.py" hl_lines="4 9"
# ...
from sqlalchemy.ext.asyncio import AsyncSession
# ...
...
```python title="fast_zero/routers/users.py" hl_lines="1 12 13"
@router.put('/{user_id}', response_model=UserPublic)
async def update_user(
    user_id: int,
    user: UserSchema,
...
!!! warning "Comportamentos estranhos"
	Esse é um comportamento que foi resolvido via [`sysmon`](https://coverage.readthedocs.io/en/latest/config.html#run-core) na versão 3.14 do Python. Caso esteja no 3.14, esse comportamento estranho não ocorrerá e a cobertura será a mesma tanto async, quanto sync. Contudo, se estiver usando uma versão menor, provavelmente terá que lidar com a correção desse comportamento descrita nesse tópico.
...
Assim, voltamos aos 88% que tínhamos antes desta aula:
...
```shell title="$ Execução no terminal!"
git add .
git commit -m "refactor(async): adiciona suporte a asyncio e torna o projeto não bloqueante"
```
