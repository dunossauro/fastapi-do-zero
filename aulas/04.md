...
3. `Session(engine)`: cria uma sessão `Session` para que os testes possam se comunicar com o banco de dados via `engine`.
...
Para fazer a validação de todos os campos do objeto durante os testes, podemos criar um evento que será executado durante o teste que faça com que os registros inseridos nesse teste tenham o horário manipulado, facilitando a comparação com um `created_at` fixo:
...
A ideia por trás dessa função é ser um gerenciador de contexto (para ser chamado em um bloco `#!python with`). Toda vez que um registro de `model` for inserido no banco de dados, se ele tiver o campo `created_at`, por padrão, o campo será cadastrado com a sua data pré-fixada '01/01/2024'. Facilitando a manutenção dos testes que precisam da comparação de data, pois será determinística.
...
```shell title="$ Execução no terminal!"
git commit -m "feat(db): adiciona primeira migração Alembic e cria tabela de usuários"
```
