...
Uma menção especial ao campo `init`. Ele diz que na hora em que o objeto for instanciado, ele não precisa ser passado, para que o SQLAlchemy atribua valores automaticamente. As chaves primárias são incrementadas automaticamente. O campo `created_ad` executa a função atrelada ao parâmetro `server_default`, no caso atribuindo `func.now()` ao campo.
