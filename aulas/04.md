# [OK] Integrando Banco de Dados a API
...
Vamos criar a nossa fixture `client` para substituir a função `get_session` que estamos injetando no endpoint pela sessão do banco em memória que já tínhamos definido para banco de dados.
...
Depois de realizar essas mudanças, podemos executar nossos testes e verificar se estão passando. Porém, embora o teste `test_create_user` tenha passado, precisamos agora ajustar os outros endpoints para que eles também utilizem a nossa sessão de banco de dados. Nos próximos passos, vamos realizar essas modificações para garantir que todo o nosso aplicativo esteja usando o banco de dados real.

## Modificando o Endpoint GET /users

{dizer que agora vamos atualizar o nosso endpoint de GET para funcionar no modelo real do banco de dados}
...
{explicar o que são as coisas que adicionamos como `offset` e `limit` e porque elas são importantes agora nesses casos}
...
{dizer que precisamos alterar o `UserPublic` que usamos no teste para que ele reconheça o ORM}
