...
- No primeiro teste com o banco, a fixture de `session` deve ser alterada para:
    1. Não checar se está na mesma thread
	2. Usar uma conexão estática `StaticPool`
	3. Essa alteração será introduzida em `[WIP] Threads e conexões`
...
#### [WIP] Threads e conexões

{Aqui devo explicar o por que do testclient e o teste rodarem em threads diferentes e adicionar a fixture de session a seguinte linha na engine `connect_args={'check_same_thread': False},`}

...

{Aqui devo explicar que o fato da engine abrir N conexões, precisamos garantir que as duas threads usem o mesmo canal de comunicação, adicionando `poolclass=StaticPool,` a engine}

...

{no final nossa fixture deve ser assim}:
...
{adicionar aqui uma nova execução dos testes!}
