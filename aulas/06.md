...
# [WIP] Refatorando a Estrutura do Projeto

Tópico em manutenção: https://github.com/dunossauro/fastapi-do-zero/issues/42
...
#### [WIP] Alteração da validação de token

> Adicionar nomes descritivos as capturas de tela desse tópico!

Um dos pontos que devemos levar em consideração a respeito dessa alteração no router é sobre o uso do parâmetro `prefix`. O prefixo criado muda o endereço do endpoint `/token` que usamos para validar o bearer token JWT para `/auth/token`. Esse caminho está escrito de forma fixa no `OAuth2PasswordBearer` em `security.py`, fazendo com que o caminho aponte para `/token`, que era o caminho usado antes da criação do router.

Podemos observar esse erro clicando no botão `Authorize` do swagger:

![Captura de tela](assets/06_autorização_no_swagger_apontando_para_o_endereço_errado.png){: .center .shadow }

Aqui vemos que o caminho para autorização está errado. Como consequência, se tentarmos efetuar a autenticação via swagger, receberemos um erro na interface:

![Captura de tela](assets/06_erro_exibido_no_swagger.png){: .center .shadow }

O erro, porém, não é descritivo o suficiente para visualizar a fonte do problema. Retornando somente um `Auth Error`. Para visualizar de fato o que aconteceu, devemos olhar o log gerado pelo `uvicorn` no terminal:
...
Solucionar isso é relativamente simples. Temos que alterar o parâmetro `tokenUrl` na `OAuth2PasswordBearer` para contemplar as alterações no router, apontando para `/auth/token`. Faremos isso no arquivo `security.py`:

```py title="security.py" linenums="40"
oauth2_scheme = OAuth2PasswordBearer(tokenUrl='{++auth/++}token')
```

Agora, quando o swagger for executado, a autorização levará para o endpoint correto.

![Captura de tela](assets/06_autorização_no_swagger_apontando_para_o_endereço_correto.png){: .center .shadow }
