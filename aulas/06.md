...
Primeiro, aplicaremos a autenticação no endpoint PUT. Se o `user_id` da rota não corresponder ao `id` do usuário autenticado, retornaremos um erro 409. Se tudo estiver correto, o usuário será atualizado normalmente.
...
Agora, aplicaremos a autenticação no endpoint DELETE. Semelhante ao PUT, se o `user_id` da rota não corresponder ao `id` do usuário autenticado, retornaremos um erro 409. Se tudo estiver correto, o usuário será deletado.
