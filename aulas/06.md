...
![Captura de tela de nosso token na página de debugger do jwt.io](assets/05_print_do_jwtio.png){: .center }
...
A função `create_access_token` é responsável por criar um novo token JWT que será usado para autenticar o usuário. Ela recebe um dicionário de dados, adiciona um tempo de expiração ao token (baseado na constante `ACCESS_TOKEN_EXPIRE_MINUTES`). Esses dados, em conjunto, formam o **payload** do JWT. Em seguida, usa a biblioteca `jose` para codificar essas informações em um token JWT, que é então retornado.
...
Embora esse código será coberto no futuro com a utilização do token, é interessante criarmos um teste para essa função com uma finalidade puramente didática. De forma que vejamos os tokens gerados pelo `jose` e interagirmos com ele.
...
```python title="fast_zero/app.py" hl_lines="16"
from fast_zero.security import get_password_hash
...
```python title="fast_zero/app.py" hl_lines="15 17 18"
from fast_zero.security import (
    create_access_token,
    get_current_user,
    get_password_hash,
...
```python title="tests/test_app.py" hl_lines="3 4 20 21 22 23" linenums="44"
