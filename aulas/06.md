...
A função `create_access_token` é responsável por criar um novo token JWT que será usado para autenticar o usuário. Ela recebe um dicionário de dados, adiciona um tempo de expiração ao token (baseado na constante `ACCESS_TOKEN_EXPIRE_MINUTES`). Esses dados, em conjunto, formam o **payload** do JWT. Em seguida, usa a biblioteca `pyjwt` para codificar essas informações em um token JWT, sendo então retornado.

Vale lembrar que nossa constante `SECRET_KEY` é utilizada para assinar o token. Embora só esteja sendo usada nesse momento para entender o comportamento, seria bom gerar uma chave segura e não conhecida por pessoas de fora da aplicação. Fazendo com que ninguém possa validar a assinatura da chave e forjem tokens para se passarem pela nossa aplicação.

Para isso, o Python conta com a biblioteca nativa [`secrets`](https://docs.python.org/pt-br/3.14/library/secrets.html){:target="_blank"} focada exclusivamente em gerar segredos.
...
Isso gerará um token de 32 caracteres, que você pode usar para substituir sua `SECRET_KEY`. Talvez ainda não seja um bom momento para fazer isso, pois ela irá ser *commitada* no seu repositório do git. Mas, vamos resolver isso na [aula 07](07.md#adicionando-as-constantes-a-settings){:target="_blank"}.
...
git status
git add .
git commit -m "feat(auth): protege endpoints PUT e DELETE com autenticação"
git push
```
