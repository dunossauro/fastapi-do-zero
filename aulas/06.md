...
Uma coisa que pode ter passado em branco durante a faze de testes é a validação do token JWT. Ele pode estar em um formato inválido, às vezes por erro do cliente, outras por algum problema de segurança. É importante validarmos como a aplicação vai reagir a um token inválido.
...
Usamos um assert para validar se o código é mesmo `401` e outro para validar nossa mensagem de erro. Assim, temos uma garantia que, caso alguém envie um token inválido, a resposta seguirá como esperado.
...
1. Faça um teste para cobrir o cenário que levanta exception `credentials_exception` na autenticação caso o `User` não seja econtrado. Ao olhar a cobertura de `security.py` você vai notar que esse contexto não está coberto.
