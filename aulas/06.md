...
> Testar o swagger e mostrar que essa alteração leva ao caminho errado do token. Mostrar print do swagger, mostrar o erro no terminal da aplicação e enfim mostrar a solução em `security.py`

> Texto provisório para esse tópico!

Por consequência a alteração do caminho do endpoint de token, a validação do OAuth encaminhará a chamada do `Bearer` na validação do JWT para o caminho errado! Pois a validação do token em `security.py` aponta para `/token`. Isso precisa ser alterado em relação ao router que adiciona o prefixo `/auth` para os endpoints de autorização e autenticação, para que a validação seja feita no caminho correto!

```py title="security.py" linenums="40"
oauth2_scheme = OAuth2PasswordBearer(tokenUrl='{++auth/++}token')
```

Desta forma, quando tentarmos usar o swagger, ele apontará para o lugar certo.
