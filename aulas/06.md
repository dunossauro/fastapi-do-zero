...
def create_user(user: UserSchema, session: Session = Depends(get_session)):
    ...
    if db_user:
        raise HTTPException(status_code=400, detail='Email already registered')  #TODO
...
def update_user(
    ...
    if db_user is None:
        raise HTTPException(status_code=404, detail='User not found')  #TODO
...
def login_for_access_token(
    ...
    if not user:
        raise HTTPException(
            status_code=400, detail='Incorrect email or password'  # TODO
        )
