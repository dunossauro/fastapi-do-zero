---
title: Criando rotas CRUD para gerenciamento de tarefas
...
1. Ao introduzir a string `#!py 'selectin'` temos um falso positivo de grafia:
    ...
    Agora tudo deve estar certo para executar os testes novamente e passando com sucesso pela etapa de `pre_lint` do taskipy. # TODO
...
Esse comportamento acontece por termos usado [`#!python lazy='selectin'`](#__codelineno-2-33){:target="_blank"} no `relationship`. Isso faz com que o novo campo traga os resultados de `Todo`s associados a esse `User` quando for buscado via `select`. Por esse motivo devemos alterar nosso teste de criação de users:
...
```shell title="$ Execução no terminal!"
poetry test tests/test_todos.py
# ...
...
Outra coisa que pode ser feita, além de deixar cada filtro opcional na querystring é utilizar o campo `Field` do pydantic, como fizemos na [aula 07](07.md#restringindo-queries-com-field). Desta vez, porém, aplicando limitações às strings.
...
Desta forma, `#!python Field(None, min_length=3, max_length=20)`, podemos garantir que a requisição feita deve ter ao menos `3` caracteres e no máximo `20`.

Embora essa seja uma mudança sútil, evitam-se buscas por uma quantidade grande de registros no resultado. Imagine a busca por um único caractere, como `a`. Ela pode ser bastante exaustiva, trazendo uma quantidade grande de volume de dados na resposta. Embora os `limit` e `offset` estejam no filtro, limitando o tamanho da resposta, essas buscas ainda podem ser bastante extensas.

![Swagger mostrando os queryparams com filtros](assets/10/swagger_queryparams_filters.png){: .center .shadow }

???+ note "URLs longas"
    Embora URLs não tenham uma limitação especificada em seu tamanho, os navegadores têm limitações em relação ao tamanho delas. Nada extremamente preocupante. O falecido IE explorer, por exemplo, tinha uma limitação de 2.083 caracteres. Mas… sempre que possível, é importante limitar o tamanho máximo, pode evitar bugs e até mesmo alguns códigos `414` de erro.
...
2. Commit as mudanças: `git commit -m "feat(tasks): implementa endpoints de tarefas"`
3. Suba suas alterações: `git push`

## Suplementar / Para próxima aula

Na próxima aula vamos criar containers docker da nossa aplicação caso não tenha nenhuma experiência com docker a [Linuxtips](https://www.youtube.com/playlist?list=PLf-O3X2-mxDn1VpyU2q3fuI6YYeIWp5rR){:target="_blank"} tem uma playlist completa e grátis sobre docker no canal deles no YouTube)
...
5. Crie um teste para validar o caso do `Enum` em [`state: Mapped[TodoState]`](#__codelineno-2-44){:target="_blank"} na tabela `TODO`, onde o valor esteja fora dos valores mapeados por ele. Isso forçará um erro que pode ser validado com [`pytest.raises`](https://docs.pytest.org/en/4.6.x/reference.html#pytest-raises){:target="_blank"}
