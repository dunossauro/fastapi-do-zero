...
> Explicar o que são os secrets do repositório no github, como eles se relacionam com as variáveis de ambiente.
...
> Explicar essa configuração

> Dizer que existem duas formas de configurar os segredos no repositório. Podemos ir na guia do repositório no github `Settings -> Secrets and variables` ou então fazer via gh e adicionar nossos segredos todos de uma vez

```shell title="$ Execução no terminal!"
gh secret set -f .env
```

> Agora dizer que com isso podemos executar nosso CI novamente. E para isso temos que fazer um novo commit para fazer o trigger
...
> Dizer que se formos olhar novamente, o CI continua quebrado, pois na aula passada modificamos os testes para que fosse executados com o postgres

> Adicionar aqui um print do erro no CI
