...
SessionDep = Annotated[Session, Depends(get_session)] #(1)!
CurrentUser = Annotated[User, Depends(get_current_user)]
```

1. A escolha pelo nome `SessionDep` para o nome é que criaremos uma DEPendência de `Session`. Não utilizaremos `Session` como nome do tipo, pois ele conflitaria com o nome já existente no namespace.
...
@router.get('/', response_model=UserList)
def read_users(session: SessionDep, filter_users: Annotated[FilterPage, Query()]):
    ...
```
...
Como conversamos na [aula 06](06.md#gerando-tokens-jwt){:target="_blank"}, precisamos de uma `SECRET_KEY` que realmente seja um segredo, então agora pode ser uma boa hora para gerar uma usando `secrets`:

```python title=">>> Terminal interativo!"
>>> import secrets
>>> secrets.token_hex(32)
```

{% include "templates/key_segura.md" %}

Dessa forma, alterando a chave `SECRET_KEY` para uma um pouco mais segura e que não será commitada no repositório:

```shell title=".env" linenums="2"
SECRET_KEY="9ec03f2fa8103aefd3dcebe0790588d264b33c729e2b2110f439d20994adfd74"
```

> Essa chave é um exemplo, evite utilizar essa. Você pode gerar a sua até mesmo aqui na página, mas lembre-se de que deve ser uma chave sua.
...
```shell title="$ Execução no terminal!"
git add .
git commit -m "refactor: reorganiza estrutura do projeto com routers de users/auth e variáveis de ambiente"
```
