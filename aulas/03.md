...
Quando relacionamos um modelo a resposta do enpoint, o Pydantic de forma automática, cria um schema chamado `HTTPValidationError`:

![descrição](assets/03/422_no_swagger.png){: .center .shadow }

Esse modelo é usado para quando o JSON enviado na requisição não cumprir os requisitos do schema.
...
Para resolver esse problema e evitar o erro 500, precisamos completar a implementação do nosso endpoint. Isso envolve adicionar a lógica necessária para processar o input, criar ou modificar os dados conforme solicitado e, finalmente, retornar uma resposta que esteja em conformidade com o modelo de resposta definido (`UserPublic`). Dessa forma, garantimos que a resposta do nosso endpoint seja validada corretamente pelo Pydantic, oferecendo uma interação segura e previsível com a API.

## [RPA] De volta ao POST
