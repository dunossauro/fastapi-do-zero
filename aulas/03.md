...
??? info "Caso fixtures sejam uma novidade para você"
	Existe uma live de python onde conversamos especialmente sobre fixtures
...
No arquivo `conftest.py`, vamos introduzir uma fixture aos nossos testes pytest chamada "session". uma fixture é uma função que permite definir e reutilizar configurações de preparação para testes ou outras funções. Elas são úteis para quando você tem partes do código que precisam ser reutilizadas em vários testes.
...
1. Em `#!python create_engine('sqlite:///:memory:', connect_args={'check_same_thread': False}, poolclass=StaticPool)`: estamos criando um mecanismo de banco de dados SQLite em memória usando o SQLAlchemy. Este mecanismo será usado para criar uma sessão de banco de dados para nossos testes.
     - Estamos usando `#!python connect_args={'check_same_thread': False}` porque SQLite tem algumas limitações quando usado em diferentes threads.
	 - E o `#!python poolclass=StaticPool` significa que a mesma conexão será reutilizada para toda a duração do teste.

2. `#!python Session = sessionmaker(autocommit=False, autoflush=False, bind=engine)`: Aqui estamos criando uma fábrica de sessões que será usada para criar sessões de banco de dados para nossos testes. Definimos `autocommit=False` para que possamos controlar quando as transações são commitadas e `autoflush=False` para que o SQLAlchemy não tente automaticamente flush (isto é, sincronizar o estado da sessão com o banco de dados) a cada operação.

3. `#!python Base.metadata.create_all(engine)`: Estamos criando todas as tabelas no banco de dados de teste. Isso é feito antes de cada teste que usa a fixture `session`.

4. `#!python yield Session()`: Estamos produzindo uma instância de Session que será injetada em cada teste que solicita a fixture `session`. Essa sessão será usada para interagir com o banco de dados de teste.

5. `#!python Base.metadata.drop_all(engine)`: Finalmente, após cada teste que usa a fixture `session`, todas as tabelas do banco de dados de teste são eliminadas. Isso garante que cada teste é executado contra um banco de dados limpo.

Em resumo, esta fixture está configurando e limpando um banco de dados de teste para cada teste que o solicita. Isso garante que cada teste é isolado e tem seu próprio ambiente de banco de dados limpo para trabalhar. Isso é uma boa prática em testes de unidade, onde queremos que cada teste seja independente e não afete os outros.
