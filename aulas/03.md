...
Boas-vindas de volta à nossa construção de uma aplicação usando FastAPI. Na última aula, configuramos todo o ambiente necessário para começarmos a desenvolver nosso aplicativo. Nesta aula, daremos um passo adiante ao estruturarmos os primeiros endpoints da nossa API, focando nas quatro operações básicas de gerenciamento de usuários. Vamos abordar o **CRUD** - Criar, Ler, Atualizar e Deletar - e como essas operações se aplicam aos dados da nossa base de dados.

Antes de prosseguirmos com os códigos, temos que conversar sobre alguns conceitos que serão bastante importantes para quem está desenvolvendo sua primeira aplicação web.
...
O HTTP é baseado no modelo de requisição-resposta: o cliente faz uma requisição para o servidor, e o servidor responde a essa requisição. Essas requisições e respostas são formatadas conforme as regras do protocolo HTTP.

A seguir, exploraremos como os verbos HTTP, os códigos de resposta e os códigos de erro são utilizados para gerenciar a comunicação entre o cliente e a API.
...
O uso do Pydantic nos permite definir modelos de dados, ou "esquemas", com campos anotados com tipos de dados. O Pydantic garante que as instâncias desses modelos sempre estejam conforme o esquema definido.
...
**Validação de dados**: Este é o processo de verificar se os dados recebidos estão conforme as regras e restrições definidas. Por exemplo, se esperamos que o campo "email" contenha um endereço de e-mail válido, a validação de dados garantirá que os dados inseridos nesse campo de fato correspondam a um formato de e-mail válido.

Considere um exemplo onde recebemos o seguinte objeto JSON, representando um novo usuário que quer se registrar em nosso serviço:
...
- **C**reate (Criar): Adicionar novos registros ao banco de dados. No HTTP, essa ação é geralmente associada ao verbo POST.
- **R**ead (Ler): Recuperar registros existentes do banco de dados. No HTTP, essa ação é geralmente associada ao verbo GET.
- **U**pdate (Atualizar): Modificar registros existentes no banco de dados. No HTTP, essa ação é geralmente associada ao verbo PUT ou PATCH.
- **D**elete (Excluir): Remover registros existentes do banco de dados. No HTTP, essa ação é geralmente associada ao verbo DELETE.
...
É importante notar que, ao trabalhar com FastAPI e Pydantic, nossos esquemas desempenham um papel vital na operação de "Create" (criar). Ao usar a operação POST para adicionar um novo registro ao nosso banco de dados, aproveitaremos a validação de dados do Pydantic para garantir que o novo registro esteja conforme o esquema do nosso modelo de dados. Se os dados enviados na solicitação POST não passarem na validação do Pydantic, nossa API retornará um código de status HTTP 422 (Unprocessable Entity), indicando que os dados fornecidos são inválidos ou incompletos.
...
Na nossa API, criaremos rotas correspondentes para cada uma das operações do CRUD, começando com a operação "create" (criar), que será implementada pela rota POST.
...
Para a criação dessa rota, usaremos de base o JSON que criamos anteriormente. Para que a pessoa se cadastre na nossa plataforma, ela precisa enviar os dados de nome de usuário, email e senha:
...
Agora criaremos nosso endpoint que esperará receber esse esquema Pydantic e retornará 201, caso o JSON enviado seja válido:
...
![descrição](assets/02_respsota_do_swagger.png){: .center }
...
![descrição](assets/02_swagger_sem_senha.png){: .center }
...
![descrição](assets/02_cobertura_get_sem_testes.png){: .center }
