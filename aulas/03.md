...
## [WIP] Adicionando logs para erros

> TODO: Introduzir o assunto logs e sua importância para uma API

> TODO: Adicionar link de live de logs um ad

```py title="fast_zero/app.py"
...
app = FastAPI()
logger = getLogger('uvicorn.error') #(1)!
```

1. Nessa linha pegando o handler do uvicorn responsável por escrever mensagens no shell, junto com as mensagens de log que já são exibidas normalmente.

> TODO: Explicar o efeito dessa configuração

> TODO: Mostrar onde isso vai no código

```py title="fast_zero/app.py" hl_lines="4"
@app.put('/users/{user_id}', response_model=UserPublic)
def update_user(user_id: int, user: UserSchema):
    if user_id > len(database) or user_id < 1:
        logger.info(f'User with {user_id=} not found in the database') #(1)!
        raise HTTPException(
            status_code=HTTPStatus.NOT_FOUND, detail='User not found'
        )
```

1. Aqui adicionamos um registro de log no level `info`, informando o motivo do `#!python 404`

> TODO: Comentar o que essa adição realmente faz


> TODO: Adicionar captura de tela do terminal exibindo o log quando a chamada ao endpoint retorna um erro.

> TODO: Conclusão sobre o assunto


### Testando os logs

> TODO: Introduzir a importância dos testes validarem as mensagens de log

```py title="tests/test_app.py" linenums="1" hl_lines="2 7 8 19"
from http import HTTPStatus
...
def test_update_user_not_found_should_log(client, caplog): #(1)!
    caplog.set_level(INFO) #(2)!
    ...
    _, level, msg = caplog.record_tuples[0] #(4)!
    assert level == INFO #(5)!
    assert msg == 'User with user_id=666 not found in the database' #(6)!
```

1. A fixture `caplog` deve ser passada ao teste.
2. Por padrão a fixture usa o level `WARNING` para capturar os logs. Fizemos uma alteração para capturar o level que estamos tentando testar.
3. Note que o id do user passado é `#!python '666'`.
4. `.record_tuples` é uma lista de tuplas com todas as chamadas de logs que foram feitas durante o teste. Por padrão, cada tupla conta com os valores `(handler, level, mensagem)`.
5. Aqui validamos se o log que procuramos é mesmo do level `INFO`
6. Validamos a mensagem que colocamos no endpoint e o valor de `user_id` que foi chamado no teste.

> TODO: Explicar o que esse teste efetivamente testa
...
3. Adicionar uma chamada de log para o endpoint de DELETE e escrever um teste para ele
4. Criar um endpoint de GET para pegar um único recurso como `users/{id}` e fazer seus testes para `#!python 200` e `#!python 404` (os logs devem ser incluídos).
