...
Uma forma de entender esse comportamento é executando um único teste, que não seja o primeiro, isoladamente. O pytest pode chamar um uníco teste em um arquivo, se o nome do teste for seguido de um `::`. Algo parecido com isso:

```shell title="$ Execução no terminal!"
pytest tests/test_app.py::test_read_users
```

No nosso caso, como estamos usando o comando de `task`, podemos fazer assim:
...
Isso quer dizer ele deu um GET em `/users`, mas não recebeu o dado esperado. Ou seja, temos a visão explícita da interdependência dos testes.
...
Dessa forma, sempre que quisermos checar se os testes funcionam em ordem aleatórias, podemos executar os testes assim:
...
Dessa forma, testes interdependentes falharão, pois a ordem muda durante a execução. Embora tenhamos instalado o `pytest-random-order`, ainda não vamos adicionar ele ao nosso fluxo padrão de testes, pois eles falharão sempre, enquanto a situação do banco de dados real não for resolvida na aula [aula 05](05.md/#testando-o-endpoint-post-users-com-pytest-e-fixtures){:target="_blank"}.
