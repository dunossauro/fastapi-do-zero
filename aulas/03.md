...
Antes de fazermos o commit, é uma boa prática checarmos todo o código, e podemos fazer isso com as ações que criamos com o poe.
...
Todos os testes que fizemos até esse momento são ótimos, eles realmente testam o que deveriam testar. Porém, existe um problema na dinâmica em que eles acontecem. Esses testes só funcionam se forem executados na exata ordem em que foram escritos. Existe uma interdependência entre eles.

Atualmente, os testes são executados nesta ordem:

- Teste 1: cria um `User` na "base de dados" e valida se ele foi criado
- Teste 2: lê um `User` existente na base de dados
- Teste 3: altera um `User` existente na base de dados
- Teste 4: deleta um `User` existente na base de dados

Ou seja, para que todos os testes passem, o primeiro precisa passar. Isso quer dizer que os testes não são isolados. Eles não dependem somente deles para serem executados.

Uma forma de entender esse comportamento é executando um único teste, que não seja o primeiro, isoladamente. O pytest pode chamar um teste em um arquivo específico, se o nome do teste for seguido de um `::`. Algo como:

```shell title="$ Execução no terminal!"
pytest tests/test_app.py::test_read_users
```

No nosso caso, como estamos utilizando o comando de `test`, podemos fazer assim:
...
Isso quer dizer que ele deu um GET em `/users`, mas não recebeu o dado esperado. Ou seja, temos a visão explícita da interdependência dos testes.

---

Na implementação atual, não é possível fugir disso sem dar um contorno grande e desnecessário na implementação, já que não vamos manter esse banco "falso", o que queremos é exatamente o oposto disso, é utilizar um banco de dados real.

Para o isolamento funcionar adequadamente, precisaremos de um mecanismo que reinicie o banco de dados a cada teste, mas ainda não temos um banco de dados real. Ele será introduzido na aplicação na [aula 04](04.md){:target="_blank"}, ainda com algumas interferências. O mecanismo que fará com que os testes não interfiram em outros e sejam independentes será introduzido na [aula 05](05.md/#testando-o-endpoint-post-users-com-pytest-e-fixtures){:target="_blank"}.


### Teria como saber disso antes?

Sim, é possível descobrir isso simplesmente trocando a ordem dos testes. O problema é que variar a ordem manualmente poderia acarretar outros casos parecidos. Randomizar a execução de todos os testes é uma boa solução, fazendo com que esses casos não ocorram mais, sem depender do conhecimento tácito de todos os testes por quem os implementou.

O pytest tem um plugin para isso chamado [pytest-random-order](https://github.com/pytest-dev/pytest-random-order){:target="_blank"}, que pode trocar a ordem de todos os testes a cada execução:

```shell title="$ Execução no terminal!"
poetry add --group dev pytest-random-order
```

Dessa forma, sempre que quisermos checar se os testes funcionam em ordem aleatória, podemos executar os testes assim:

```shell title="$ Execução no terminal!"
poetry test --random-order
```

E nosso resultado será algo parecido com:
...
1. Note, que como a ordem é randômica, outro teste pode ter falhado na sua execução.

Dessa forma, testes interdependentes falharão, pois a ordem muda durante a execução. Embora tenhamos instalado o pytest-random-order, ainda não vamos adicioná-lo ao nosso fluxo padrão de testes, pois falharão sempre, enquanto a situação do banco de dados real não for resolvida na [aula 05](05.md/#testando-o-endpoint-post-users-com-pytest-e-fixtures){:target="_blank"}.

> Uma coisa legal a se fazer aqui é rodar várias vezes o comando e ver falhar diferentes :)
...
```shell title="$ Execução no terminal!"
git commit -m "feat: implementa rotas CRUD"
```
