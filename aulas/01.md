...
O poe tem várias formas de ser instalado, no projeto via poetry, globalmente via pipx, ou como um plugin do poetry, que é como vamos instalar:

```toml title="pyproject.toml" linenums="52"
[tool.poetry.requires-plugins]  #(1)!
poethepoet = { version = "~0.48.0", extras = ["poetry_plugin"]}

[tool.poe] #(2)!
poetry_command = ""
```

1. Indica ao Poetry que este projeto necessita que o plugin do poethepoet esteja instalado para funcionar
2. Configuração para o poe entender que ele deve se integrar diretamente como um comando nativo do Poetry (permitindo rodar `poetry serve` em vez de `poetry poe serve`).
