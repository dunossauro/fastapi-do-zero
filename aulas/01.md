...
Para configurar o ruff, definiremos o comprimento de linha para 79 caracteres (conforme sugerido na [PEP-8](https://peps.python.org/pep-0008/){:target="_blank"}) e em seguida, informaremos que o diretório de migrações de banco de dados deverão ser ignorados:
...
> TODO: Escrever sobre o formatador
...
> TODO: Atualizar a lista de comandos pois mudamos para o ruff
...
=== "Comando"
    > TODO: Alterar essa mensagem para a mensagem do ruff
...
```python title="tests/test_app.py" linenums="1"
--8<-- "aulas/codigos/01/tests/test_app.py:6:19"
```

> TODO: Ajustar o ponto 1
...
4. Aqui fazemos a validação do código de resposta, para saber se a resposta é referente ao código `200`, que significa `OK`.
5. No final, validamos se o dicionário que enviamos na função é o mesmo que recebemos quando fizemos a requisição.
6. Importação da biblioteca nativa do python que abstrai os códigos de resposta e nos apresenta o status.
...
```python title="tests/test_app.py" linenums="1"
--8<-- "aulas/codigos/01/tests/test_app.py:20"
```
