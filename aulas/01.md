...
Certifique que o python rodando no seu ambiente é mesmo o 3.11:
...
A resposta esperada é `Python 3.11.3`.
...
Para garantir que tudo está configurado corretamente, vamos criar um pequeno programa "Hello, World!" com FastAPI. Em um novo arquivo chamado `main.py`, adicione o seguinte código:
...
Para instalar as dependências, podemos usar um grupo do poetry focado nelas, para que não sejam usadas em produção:
...
Vamos configurar o isort para o formato do Black, que coloca um import por linha para evitar conflitos no git e também usar a base da PEP-8 para 80 caracteres:
...
??? warning "Caso precise do arquivo todo"
...
Antes de entendermos a dinâmica dos testes, precisamos entender o efeito que eles tem no nosso código. Um bom lugar para começar isso é analisando a cobertura. Vamos executar os testes.
...
A parte importante dessa Mensagem está na tabela gerada pelo `coverage`. Que diz que temos 5 linhas de código (Stmts) no arquivo `fast_zero/app.py` e nenhuma delas está coberta pelos nosso testes. Como podemos ver na coluna `Miss`.
