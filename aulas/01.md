...
O [pipx](https://pipx.pypa.io/latest/){:target="_blank"} é uma ferramenta usada para instalar/executar ferramentas python de forma global no sistema. Diferente do `pip`, que instalaria ferramentas sem um ambiente virtual e acaba "sujando" nosso ambiente, o `pipx` cria um ambiente virtual e isola cada ferramenta dentro dele. Fazendo com que a instalação de pacotes globais seja facilitada.

Em nosso projeto, usaremos o pipx para instalar ferramentas globais e executar algumas ferramentas que serão usadas somente uma vez.
...
Dessa forma, a única dependência global que teremos no nosso sistema é o próprio `pipx`.

??? warning "Existem outras formas de instalar o pipx"
	Caso você tenha um gerenciador de pacotes no seu sistema operacional, é extramente recomendado que você instale o pipx por ele.
	
	As intruções de instalação para cada sistema estão disponíveis na [documentação do projeto](https://pipx.pypa.io/latest/installation/#installing-pipx){:target="_blank"}.


Para que o nosso sistema reconheça o caminho das ferramentas instaladas via `pipx` podemos executar o comando:

```shell title="$ Execução no terminal!"
pipx ensurepath #(1)!
```

1. Adiciona o diretório dos ambientes virtuais do pipx no path do sistema.

Esse comando adiciona no `PATH` do sistema todos os binários instalados pelo `pipx` 🚨. Portanto, lembre-se de reiniciar o shell após isso.


## Poetry

O [Poetry](https://python-poetry.org/){:target="_blank"} é um gerenciador de projetos para Python. Ele pode nos ajudar em diversas etapas do ciclo de desenvolvimento. Como:
...
A instalação do poetry pode ser feita de diversas maneiras diferentes. A forma que **recomendo**, para instalação global, mas isolada em um ambiente virtual, é via `pipx`:
...
???+ danger "Comentários em blocos"
	Blocos de código costumam ter comentários com informações adicionais, como esse:
...
1. A função `self add` do poetry instala extensões para podermos executar novos comandos com o poetry, nesse caso `poetry shell` para entrarmos nos ambientes virtuais.

Essa extensão instala um comando adicional ao poetry, o `poetry shell`, que habilita o ambiente virtual no terminal.


### Gerenciamento de versões do python

Após a instalação do poetry, podemos usar ele pra gerenciar/instalar versões do python que gostaríamos de usar em um projeto. Para acompanhar esse curso, a versão mínima do python que você deve ter é a `#!python 3.11`. Pois alguns recursos que usaremos foram introduzidos nessa versão.

Você pode, porém, instalar qualquer versão mais nova que ela. Minha recomendação, é **sempre que possível**, use a versão mais atualizada possível:
...
Dessa forma temos uma versão compatível do python instalada.

## Criando um projeto

Agora que temos o poetry e a versão do python que usaremos disponível, podemos iniciar a criação do nosso projeto. Inicialmente criaremos um novo projeto usando o Poetry, com o comando `poetry new` e em seguida navegaremos até o diretório criado:
...
Com a estrutura inicial do projeto criada e dentro do diretório do projeto, podemos dizer ao poetry que queremos usar a versão do python que instalamos no projeto:
