...
Para fazer a instalação do Poetry de forma global, mas isolada em um ambiente virtual, em sistema, vamos usar o `pipx`:
...
1. A função `inject` do pipx instala uma dependência em um ambiente virtual já existente. Nesse caso vamos instalar o `poetry-plugin-shell` no ambiente virtual do `poetry`.
