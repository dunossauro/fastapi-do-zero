...
A linha em destaque `#!python @app.get('/')` expõem a nossa função para ser servida pelo FastAPI. Dizendo que quando um cliente acessar o nosso endereço de rede no caminho `/`, usando o método HTTP GET[^1], a função será executada. Assim, já escrevemos nossa primeira aplicação FastAPI.

[^1]: Exploraremos mais a fundo a relação de métodos e o protocolo HTTP na próxima aula.
...
![Figura representando a divisão para "protocolo" com "http://" , "endereço" com "127.0.0.1" e "porta" com ":8000"](assets/01_endereco_http_black.png#only-light){: .center .shadow }
![Figura representando a divisão para "protocolo" com "http://" , "endereço" com "127.0.0.1" e "porta" com ":8000"](assets/01_endereco_http_white.png#only-dark){: .center .shadow }
...
![Captura de tela do browser acessando "http://127.0.0.1"](assets/01_nagevador_com_localhost.png){: .center .shadow }
...
??? example "Diferentes clientes para nossa aplicação"
	...
	![Captura de tela do hoppscotch](assets/01_cliente_grafico_para_apis_hoppscotch.png){: .center .shadow }

	Ou como o [Bruno](https://www.usebruno.com/){:target="_blank"}:
	![Captura de tela do bruno](assets/01_cliente_grafico_para_apis_bruno.png){: .center .shadow }
