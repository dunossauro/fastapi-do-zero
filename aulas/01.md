...
```shell title="$ Execução no terminal!"
pyenv local 3.11.7  # Essa era a maior versão do 3.11 quando escrevi
```
...
Esse arquivo fará com que toda vez que o terminal for aberto nesse diretório, o pyenv usará a versão descrita no arquivo quando o python interpretador for chamado.
...
> Para para a execução do servidor no shell, você pode digitar ++ctrl+c++ e a mensagem `Shutting down` aparecerá mostrando que o servidor foi finalizado
...
Para testar o código feito com FastAPI, precisamos de um cliente de teste. A grande vantagem é que o FastAPI já conta com um cliente de testes no módulos `fastapi.testclient` com o objeto `TestClient`, que precisa receber nosso app como parâmetro:
