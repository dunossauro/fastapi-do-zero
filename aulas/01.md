...
???+ warning "Existem outras formas de instalar o pipx"
	...
	As instruções de instalação para cada sistema estão disponíveis na [documentação do projeto](https://pipx.pypa.io/latest/how-to/install-pipx.html){:target="_blank"}.
...
### Ambiente virtual local [opcional]

!!! warning "Passo opcional!"
	Esse passo é opcional! Caso você já utilize o Poetry com frequência ou tenha familiaridade com a configuração do seu editor de texto, pode pular essa etapa.

	Se quiser saber mais sobre isso, consulte a [documentação do Poetry](https://python-poetry.org/docs/configuration#virtualenvsin-project){:target="_blank"} sobre essa configuração.

Por padrão, o Poetry cria ambientes virtuais em um diretório centralizado fora do projeto. Isso é ótimo em diversos cenários, principalmente no desenvolvimento de bibliotecas e quando trabalhamos com múltiplos projetos.

No entanto, alguns editores de texto podem exigir configurações adicionais para reconhecer automaticamente o ambiente virtual criado pelo Poetry. Quando isso não acontece, é comum aparecerem avisos de importação ou erros falsos no editor, mesmo com o projeto funcionando corretamente. O sintoma mais comum são aquelas "minhoquinhas" vermelhas aparecendo abaixo dos imports. Algo como:

![Captura de tela do meu editor de texto exibindo erros por não encontrar o import do FastAPI](assets/01/minhoquinhas.png){: .center .shadow }

Para evitar esse tipo de problema, podemos configurar o Poetry para criar o ambiente virtual dentro do próprio projeto, em um diretório chamado `.venv`:

```bash
poetry config virtualenvs.in-project true
```

Com isso, muitos editores conseguem detectar automaticamente o ambiente virtual do projeto, reduzindo a necessidade de configurações adicionais.
...
Assim, podemos iniciar o servidor de desenvolvimento com:

```shell title="$ Execução no terminal!"
poetry run fastapi dev fast_zero/app.py #(1)!
```

1. `poetry run` executa um comando de shell usando o virtualenv ativado.
...
- [poethepoet](https://poethepoet.natn.io/index.html){:target="_blank"}: ferramenta usada para criação de comandos. Como executar a aplicação, rodar os testes, etc.
...
```shell title="$ Execução no terminal!"
poetry add --group dev pytest pytest-cov ruff typos
```
...
Na segunda linha, usamos a opção `addopts` do pytest. Essa opção diz a ele todas às vezes em que o pytest for chamado, ele vai adicionar esse parâmetros a chamada, sem que tenhamos que os declarar explícitamente.
...
Cada parâmetro desse tem um significado importante:

- `-p no:warnings`: dizemos que não queremos mensagens de "warnings" na saída dos testes. Costuma deixar mais limpo.
- `--cov=fast_zero`: diretório onde está nosso código que será testado, para mostrar por onde os testes passam
- `--cov-context=test`: isola testes em contextos, para dizer exatamente qual teste executada cada linha de código.

Desta forma, ao testar, teremos uma visualização boa e coerente do que eles testam.

> Vamos ver essas coisas em ação [aqui](#introducao-ao-pytest-testando-o-hello-world){:target="_blank"}.
...
Como falantes de português, nem sempre queremos escrever tudo em inglês, como a documentação ou o `README.md` do repositório.

Nesses casos, podemos configurar o `typos` para ignorar determinados arquivos:
...
1. Aqui todos os arquivos (`*`) com a extensão `.md` estão sendo ignorados. O diretório `#!py "migrations"` vai ser ignorada preventivamente. Mas, esse diretório só será inserido na aula 05.

Dessa forma, o `typos` deixará de analisar arquivos Markdown, por exemplo.

### Poe the Poet

A ideia do [Poe the Poet](https://github.com/nat-n/poethepoet){:target="_blank"} (ou simplesmente `poe`) é ser um executor de tarefas (*task runner*) integrado ao nosso ecossistema de desenvolvimento. No lugar de ter que lembrar comandos longos e complexos, como os do FastAPI que vimos na execução da aplicação, que tal substituir tudo simplesmente por `poetry serve`?

Isso funciona para qualquer comando complicado em nossa aplicação, simplificando as chamadas e centralizando o fluxo de trabalho no arquivo de configuração do projeto.

O poe tem várias formas de ser instalado, no projeto via `pyproject.toml`, globalmente via `pipx`, ou diretamente como um plugin do poetry. Vamos injetar via `pipx`:

```shell title="$ Execução no terminal!"
pipx inject poetry poethepoet[poetry_plugin] #(1)!
```

1. Instala o poethepoet no mesmo venv do pipx com o poetry, estendendo as funcionalidades dele.

??? example "posso instalar de outras formas?"
	Sim, você pode: [Guia de instalação do poethepoet](https://poethepoet.natn.io/installation.html), mas para curso, o melhor caminho talvez seja via pipx, pois já instalamos ele.
...
1. Configuração para o poe entender que ele deve se integrar diretamente como um comando nativo do Poetry (permitindo rodar `poetry serve` em vez de `poetry poe serve`).

Após essa configuração inicial, podemos definir diversos comandos e agrupar diversas das nossas ferramentas simplificando bastante nosso fluxo de desenvolvimento:
...
Onde cada comando é responsável por uma, ou um grupo de operações:

- `lint`: executa o `ruff check` para garantir as boas práticas
- `format`: executa uma sequência que primeiro corrige o que for possível automaticamente (`ruff check --fix`) e depois formata o estilo do código (`ruff format`).
- `serve`: executa o servidor de desenvolvimento do FastAPI.
- `test`: executa uma sequência completa de testes: roda o `lint`, executa os testes com pytest de forma verbosa (`-vv`), permite passar argumentos extras pelo terminal através da variável `$POE_EXTRA_ARGS` e, por fim, gera o relatório de cobertura.

Para executar um comando, é bem mais simples, precisando somente passar a palavra `poetry <comando>`.

#### Entendendo Tarefas em Sequência

Repare que para os comandos `format` e `test` utilizamos a propriedade `.sequence` em vez de passar uma string direta de comando. 

No **Poe the Poet**, uma sequência nos permite encadear múltiplos comandos que serão executados de forma linear (um após o outro). Um detalhe crucial sobre o comportamento das sequências é que como elas são executadas em cadeia, se uma falhar, o poe não vai executar as seguintes.
...
Caso você tenha copiado o código que usamos para definir `fast_zero/app.py`, pode testar os comandos que criamos para o `poe`:
...
Por não encontrar nenhum teste, o pytest retornou um "erro". Isso significa que nossa tarefa sequencial `test` não foi executada. Podemos executá-la manualmente:
...
Por não coletar nenhum teste, o pytest ainda retornou um "erro". Para ver a cobertura, precisaremos executar novamente o passo de cobertura manualmente:
...
No verde, vemos o que foi executado quando a ferramenta de cobertura rodou, e no vermelho o que não foi. Outra informação importante, destacada em amarelo, é o texto `(empty)`. Ele indica que nenhum teste executou de fato essas linhas. Elas foram apenas carregadas durante o import do arquivo, e não por um teste específico. Por isso, não exibem informação de contexto.
...
Como conseguimos coletar um item, a sequência da task `test` foi executada e também gerou um HTML com a cobertura atualizada.
...
Com o repositório pronto, vamos versionar nosso código. Primeiro, adicionamos o código ao próximo commit com `git add .`. Em seguida, criamos um ponto na história do projeto com `git commit`. Por fim, sincronizamos o repositório local com o remoto no GitHub usando `git push`:
...
Você pode ter notado algo diferente nessa mensagem de commit:

```bash
chore: configuração inicial do projeto
```

Esse `chore:` no começo não é enfeite, ele faz parte de um padrão chamado [**Conventional Commits**](https://www.conventionalcommits.org/pt-br/v1.0.0/).

A ideia desse tipo de commit é bastante simples: dar significado para os commits, deixando claro *o tipo de mudança que foi feita*, só batendo o olho na mensagem.

Em vez de commits com mensages genéricas como:

- `"arruma coisa"`
- `"adicionando classe XPTO"`
- `"update"`

a gente passa a escrever algo mais estruturado:

```bash
feat: adiciona rota de usuários
...
faz sentido porque não estamos criando uma feature nem corrigindo um bug. Estamos só preparando o terreno: dependências, ferramentas, estrutura inicial.

Isso ajuda em duas coisas bem práticas:

1. Histórico mais legível: dá pra entender a evolução do projeto sem decifrar commits genéricos.
2. Automação futura: em projetos maiores, esses padrões ajudam a gerar changelog automaticamente e até facilitar [versionamento semântico](https://semver.org/lang/pt-BR/).

De forma alguma isso é obrigatório em repositórios git, mas, como estamos buscando construir boas práticas, isso pode ser um recurso interessante pra usarmos ao longo do curso.
...
2. Adicione o `typos` para analisar gramática em inglês para ser executado antes da etapa de lint, transformando ela em uma sequência que rode o `typos` e o `ruff check` na tabela `[tool.poe.tasks]` do seu arquivo `pyproject.toml`.
