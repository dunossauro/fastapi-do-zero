...
> TODO: Explicar mais coisas sobre o Ruff

O [Ruff](https://docs.astral.sh/ruff/){:target="_blank"} tem duas funções principais:
...
Durante a análise estática do código, queremos buscar por coisas específicas. No Ruff, precisamos dizer exatamente o que ele deve analisar. Isso é feito por códigos. Vamos usar estes:
...
- `PT` (Pytest): Checagem de boas práticas do Pytest

```toml title="pyproject.toml" linenums="24"
--8<-- "aulas/codigos/01/pyproject.toml:24:27"
```

> Para mais informações sobre a configuração e sobre os códigos do ruff, você pode checar a [documentação](https://docs.astral.sh/ruff/rules){:target="_blank"}
...
> TODO: Ajustar o ponto 1 dos tips
