...
Para que a versão que instalamos com pyenv seja usada em nosso criado projeto com poetry, devemos dizer ao pyenv qual versão do python será usada nesse diretório:

```title="$ Execução no terminal!"
pyenv local 3.11.4  # Essa era a maior versão do 3.11 quando escrevi
```

Em conjunto com essa instrução, devemos dizer ao poetry que usaremos essa versão em nosso projeto:
