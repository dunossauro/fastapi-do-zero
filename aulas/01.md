...
Em seguida, inicializaremos um novo projeto Python com Poetry e instalaremos as dependências necessárias. O FastAPI e o Uvicorn:
...
O FastAPI é ótimo para criar APIs, mas não pode disponibilizá-las na rede sozinho. Para que possamos acessar essas APIs através de um navegador ou de outras aplicações, é necessário um servidor. É aí que o Uvicorn entra em cena. Ele atua como esse servidor, disponibilizando a API do FastAPI em rede. Isso permite que a API seja acessada de outros dispositivos ou programas.
