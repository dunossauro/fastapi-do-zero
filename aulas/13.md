...
flyctl ssh console -a fastzeroapp -C "alembic upgrade head"
...
O subcomando que utilizamos `ssh console` nos fornece acesso ao shell do container. Por isso tivemos que especificar com a flag `-a` o nome da nossa aplicação (poderíamos acessar o console do banco de dados, também). E a flag `-C` é o comando que queremos que seja executado no console do container. Nesse caso, o comando completo representa: "Acesse o console do app fastzeroapp via SSH e execute o comando `alembic upgrade head`".
...
```shell title="$ Execução no terminal!"
git add .
git commit -m "chore: adiciona arquivos gerados pelo Fly"
git push
```
