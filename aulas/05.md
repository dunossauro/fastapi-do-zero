...
TODO: Aqui tem erro nos imports `from sqlalchemy import create_engine, event`
...
Contudo, agora que temos diversas ramificações acontecendo no código, por conta dos blocos `#!py if`, podemos experimentar mais um recurso proveniente da cobertura. A cobertura de "branch", o report dela nos ajuda a enteder os condicionais e saber quais condições foram ou não atendidas.

O que precisamos fazer é adicionar `--cov-branch` na configuração do pytest. Desta forma:
...
Agora, ao executar os testes, vemos no shell duas novas colunas na tabela `Branch` e `BrPart`:
...
`Branch` nos diz basicamente quantos `#!py if`s existem no código. Por exemplo, em `app.py` temos `#!py 12`. Junto a essa métrica, temos também `BrPart`. Que pode ser entendido com condicionais que não foram correspondidos. Por exemplo, uma expressão `#!py if` pode ser verdadeira ou falsa. Quando nossos testes passam pode somente um dos casos, ele nos diz que parte da branch não foi coberta. Ou seja, só fizemos o caso verdadeiro ou o falso.

No HTML isso fica bastante visível e ele nos diz qual parte não foi coberta, usando uma linha amarela e um aviso dizendo qual condição não foi satisfeita pelos testes:

![descrição](assets/05/coverage_mostrando_casos_nao_testados_de_erro_com_branch.png){: .center .shadow width=600 }

Ao passar o mouse pela linha amarela (hover), o report vai indicar qual das condições não foi satisfeta:

![descrição](assets/05/coverage_mostrando_casos_nao_testados_de_erro_com_branch_popup.png){: .center .shadow width=600 }

Nesse caso, se traduzida a mensagem de forma literal: "linha 64 não saltou para linha 65 porque a condição na linha 64 nunca foi verdadeira".
...
## Randomização/Isolamento de testes

Agora que temos toda essa estrutura construída via fixture `session`, garantindo que o banco de dados sempre está limpo entre os testes, é um bom momento para retomarmos a conversa sobre os isolamentos dos testes que começamos na [aula 03](03.md#isolamento-dos-testes), o real motivo de termos construído essa fixture dessa forma.

É esperado agora que os testes passem independentemente da ordem, mas precisamos garantir isso. Vamos lá:
...
Isso mostra que os testes estão isolados uns dos outros, embora você possa querer executá-los mais uma vez, só para ter certeza (claro que não é necessário, mas é divertido).

### Configurando a randomização

Se quisermos que os testes rodem em ordem randômica todas às vezes, poderíamos adicionar isso na configuração em `pyproject.toml`. Assim como acabamos de fazer com a configuração de cobertura:
...
Assim, toda vez que o pytest for chamado, pelo `poe` ou somente via `pytest`, os testes serão randomizados:
...
1. Seed gerado por essa execução.

Uma coisa importante de notar na randomização é que um novo valor de `seed` é sempre apresentado quando o teste é executado. Isso pode nos ajudar a rodar a mesma ordem no futuro. Caso exista alguma interferência gerada pela randomização. É uma camada a mais de prevenção.

---

Desta forma, é possível confirmar que a ordem dos testes não importa, pois eles estão de fato isolados. A execução de cada teste é independente uma da outra.
...
```shell title="$ Execução no terminal!"
git add .
git commit -m "feat(api): atualiza endpoints para usar banco de dados real"
git push
```
