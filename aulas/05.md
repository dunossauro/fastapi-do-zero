...
Agora que temos toda essa estrutura construída via fixture de `session`, garantindo que o banco de dados sempre está limpo entre os testes, é um bom momento para retomarmos a convera sobre os isolamentos dos testes que começamos na [aula 03](03.md#isolamento-dos-testes), o real motivo de termos construído essa fixture dessa forma.
...
Isso mostra que os testes estão isolados uns dos outros, embora você possa querer executar eles mais uma vez, só pra ter certeza (claro que não é necessário, mas é divertido)

### Configurando a randomização

Se quisermos que os testes rodem em ordem randômica todas às vezes, poderíamos adicionar isso na configuração do pytest ou do taskipy, vou na do pytest no `pyproject.toml`, para variar um pouco:
...
Assim, toda vez que o pytest for chamada, pelo `task` ou somente via `pytest`, os testes serão randomizados:
...
Uma coisa importante de notar na randomização é que um novo valor de `seed` é sempre apresentado quando o teste é executado. Isso pode nos ajudar rodar a mesma ordem no futuro. Caso exista alguma interferência gerada pela randomização. É uma camada a mais de prevenção.
