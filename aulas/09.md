...
[Código :fontawesome-solid-code:](https://github.com/dunossauro/fastapi-do-zero/tree/main/codigo_das_aulas/09/){ .md-button }
...
Primeiro, criaremos um novo arquivo chamado `todos.py` no diretório de `routes`:
...
![descrição](assets/09/cobertuda_total_dos_models_mesmo_sem_testes.png){: .center }
...
def patch_todo(
    ...
    if not db_todo:
        raise HTTPException(status_code=404, detail='Task not found.')  # TODO
