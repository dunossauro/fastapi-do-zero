...
- `--entrypoint`: A flag `--entrypoint` substitui o ponto de entrada padrão do container. O ponto de entrada padrão é especificado no `Dockerfile` ou no `docker-compose.yml` e normalmente é o comando que inicia sua aplicação. Ao substituir o entrypoint, você pode executar comandos diferentes, como neste caso, onde queremos rodar nossos testes.
...
[+] Creating 2/2
 ✔ Network 09_default                Created                      0.1s 
 ✔ Container 09-fastzero_database-1  Created                      0.1s 
[+] Running 1/1
 ✔ Container 09-fastzero_database-1  Started                      0.3s 
[+] Building 0.0s (0/0)                                           docker:default
All done! ✨ 🍰 ✨
