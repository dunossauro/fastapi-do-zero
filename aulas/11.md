---
title: Containerizando a nossa aplicação e introduzindo o PostgreSQL
description: Nossa aplicação um passo mais próximo do deploy
---

# Containerizando a nossa aplicação e introduzindo o PostgreSQL
...
Após a implementação do nosso gerenciador de tarefas na aula anterior, temos uma primeira versão estável da nossa aplicação. Nesta aula, além de aprendermos a containerizar nossa aplicação FastAPI, também abordaremos a migração do banco de dados SQLite para o PostgreSQL.


## Containers e nossa aplicação

Sempre que pensamos em distribuir uma aplicação, precisamos garantir que ela possa ser executada da mesma forma em qualquer ambiente. Uma das formas mais comuns de fazer isso é por meio de **contêineres**.

Um **contêiner** é um ambiente isolado que reúne uma aplicação com tudo o que ela precisa para funcionar, como bibliotecas, dependências e configurações. Dessa forma, a aplicação é executada da mesma maneira em qualquer computador ou servidor que possua um mecanismo de execução de contêineres instalado, independentemente do sistema operacional ou das configurações do ambiente.

Para criar e gerenciar esses contêineres, utilizaremos o [Docker](https://www.docker.com/){:target="_blank"}, uma plataforma que permite criar, distribuir e executar aplicações em contêineres. O Docker automatiza a criação e o gerenciamento desses ambientes, facilitando o desenvolvimento, os testes, a implantação e o compartilhamento de aplicações. Isso torna o processo mais consistente, reduz problemas de incompatibilidade entre ambientes e simplifica a distribuição de software.
...
Antes de começarmos, vale a pena entender alguns conceitos importantes sobre como distribuir uma aplicação em containers funciona. Normalmente seguimos três etapas:

```mermaid
flowchart LR
    A["Descrição do ambiente"] -->|build| B["Imagem"]
    B -->|run| C["Contêiner em execução"]
```

1. Descrevemos como o ambiente da aplicação deve ser construído. Essa descrição inclui o sistema operacional base, as dependências da aplicação e o comando que deverá ser executado quando ela iniciar.

2. A partir dessa descrição, uma ferramenta de conteinerização gera uma **imagem**. A imagem é um pacote imutável contendo os arquivos, dependências e configurações necessárias para criar contêineres.

3. Quando essa imagem é iniciada, ela dá origem a um **contêiner**. O contêiner é a instância em execução da imagem, onde a aplicação realmente roda.

Uma mesma imagem pode ser utilizada para criar vários contêineres independentes entre si, todos executando exatamente o mesmo ambiente.

Usaremos o Docker para realizar essas etapas. Com ele, a descrição do ambiente é feita em um arquivo chamado `Dockerfile`, a imagem é construída com o comando `docker build` e um contêiner é iniciado com `docker run`.

### Descrevendo a imagem

Para construir uma imagem de contêiner, descrevemos uma lista de passos no `Dockerfile`. Ele define o ambiente de execução, os comandos necessários para prepará-lo e o comando que será executado quando o contêiner for iniciado a partir da imagem.

Uma das coisas interessantes sobre Docker é que existe um [registro de imagens](https://hub.docker.com/){:target="_blank"} onde a comunidade publica imagens prontas para uso, que podemos usar como ponto de partida. Por exemplo, a comunidade de python mantém um grupo de imagens com o [ambiente python](https://hub.docker.com/_/python){:target="_blank"} pronto para uso. Podemos partir dessa imagem com o python já instalado e adicionar os passos para que nossa aplicação seja executada.
...
Como a aplicação será executada *dentro de* um contêiner, já temos um ambiente isolado, descartável e reproduzível. Nesse cenário, não existe a necessidade de criar outro ambiente virtual com o Poetry. Também não precisamos instalar as dependências utilizadas apenas durante o desenvolvimento e testes, apenas as necessárias para executar a aplicação. Por isso utilizamos diretamente `pip install .`, simplificando a escrita do `Dockerfile`, reduzindo o tempo de build e produzindo imagens menores, com menos dependências.

### Construindo a imagem

Para construir uma imagem de container a partir do `Dockerfile`, usamos o comando `docker build`. O comando a seguir cria uma imagem chamada "fast_zero":
...
No PostgreSQL, o diretório padrão para armazenamento de dados é `/var/lib/postgresql/`. Mapeamos esse diretório para um volume (neste caso "pgdata") em nossa máquina host para garantir a persistência dos dados:
...
4. `volumes:`: mapeia volumes para persistência de dados.
   ...
   - `pgdata:/var/lib/postgresql/`: cria ou usa um volume chamado "pgdata" e o mapeia para o diretório `/var/lib/postgresql/` no contêiner.
...
- `alembic upgrade head`: roda as migrações do banco de dados até a última versão.
- `uvicorn --host 0.0.0.0 --port 8000 fast_zero.app:app`: inicia a aplicação. Este é o comando que normalmente estaria no `CMD` do Dockerfile, mas agora está incluído no `entrypoint` para garantir que as migrações sejam executadas antes do servidor iniciar.
...
2. Faça o commit das alterações com `git commit -m "feat(docker): dockeriza aplicação e adiciona PostgreSQL"`
