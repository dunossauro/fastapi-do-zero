...
A pergunta feita ao final dessa seção `Do you want to tweak these settings before proceeding?` pode ser traduzida como: `Você deseja ajustar essas configuração antes de prosseguir?`. Vamos dizer que sim, digitando ++y++ e em seguida ++return++.
...
Nas linhas em destaque, podemos ver que o Fly se encarregou de criar um dashboard para vermos o status atual da nossa aplicação, inicializou um banco de dados postgres para usarmos em conjunto com nossa aplicação e também adicionou a url do banco de dados a variável de ambiente `DATABASE_URL` com a configuração do postgres a aplicação.

**A `Connection string` do banco de dados deve ser armazenada por você, essa informação não será disponibilizada novamente, nem mesmo na parte web do Fly. Por isso a guarde com cuidado e não compartilhem de forma alguma.**

> [WIP] Explicar mais sobre a variável de ambiente ...
...
Na ultima linha ele nos disse que nossa aplicação está pronta para o deploy. Mas ainda temos mais configurações a fazer!
...
Para vermos as variáveis que já estão configuradas no ambiente, podemos executar o seguinte comando:
...
> [WIP] Embora a variável de ambiente do banco tenha sido configurada pelo `flyctl` a variável de ambiente do banco não está correta para o uso no postgres, temos que alterar!
...
Contudo, não é somente a variável de ambiente do postgres que é importante para que nossa aplicação seja executada. Temos que adicionar as outras variáveis ao Fly.

Temos que adicionar a variável `ALGORITHM`:
...
e a variável `ACCESS_TOKEN_EXPIRE_MINUTES`:
...
Com isso, todas os segredos já estão configurados no nosso ambiente. Agora podemos partir para o nosso tão sonhado deploy

### Deploy da aplicação

O `flyctl` tem um comando específico para executarmos o deploy, o comando `deploy`.

> [WIP] Explicar que na parte o deploy pode ser feito de duas formas. Subir o container docker para o Fly ou subir o código para que o Fly faça o build do docker. A opção mais simples é fazer o build na sua máquina e subir os layers do container
