...
???+ bug "Problemas no `Windows`, pscycopg e asyncio"
	...
	O [loop de eventos](http://localhost:8080/08/#loop-de-eventos){:target="_blank"} padrão tem alguns problemas para executar as terefas com o postgres no **`Windows`**. Para contornar isso, podemos adicionar manualmente o loop de eventos do windows ao asyncio, para que tudo seja executado corretamente:
